...
(async()=>{
 ...
 try {
  for(const width of [1920,1440,768,390,320]){
   ...
   for(const [scenario,baseline,candidate] of pairs){
    for(const [name,path] of [['baseline',baseline],['candidate',candidate]]){
     const response=await page.goto('http://127.0.0.1:9816'+path);
  ...
  fs.writeFileSync('.runtime/quality/comparison.json',JSON.stringify({capturedAt:new Date().toISOString(),baseline:'be419e0255ca5cfc4b1afe4514f854e1c067161a',fixture:'same in-process SQLite records, titles and timestamps; disabled probe; all/resolved filters and recovered incident #2',scope:'geometry in CSS pixels; not productivity or user approval',checks},null,2));
