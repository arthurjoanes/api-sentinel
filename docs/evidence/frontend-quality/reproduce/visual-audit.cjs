...
(async()=>{
 ...
 for(const width of [1920,1440,768,390,320]){
  ...
  for(const route of ['/','/incidents/2','/incidents/3','/runbooks','/runbooks/erp','/__review/stale','/__review/failure','/__review/empty']){
   await page.goto('http://127.0.0.1:9816'+route);
 ...
 fs.writeFileSync('.runtime/quality/visual-audit.json',JSON.stringify({scope:'Computed text/background contrast and alignment; not a full WCAG audit',checks},null,2));
