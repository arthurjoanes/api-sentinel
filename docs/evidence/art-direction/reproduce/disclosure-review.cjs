...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});const page=await browser.newPage({viewport:{width:1440,height:1000}});try{
 await page.goto('http://127.0.0.1:9818/incidents/2?status=resolved');
 ...
 await page.screenshot({path:'.runtime/art-review/recovered-record.png',fullPage:true});
 await page.goto('http://127.0.0.1:9818/__review/index#observacao');
 await expect(page.locator('#observacao summary')).toBeFocused();
 await page.screenshot({path:'.runtime/art-review/probe-details.png',fullPage:true});
 ...
 fs.writeFileSync('.runtime/art-review/disclosure-review.json',JSON.stringify(out,null,2));console.log(out);
